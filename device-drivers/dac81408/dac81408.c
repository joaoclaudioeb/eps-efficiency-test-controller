...
#define DAC81408_SPI_INSTANCE spi1
#endif
...
int dac81408_config(dac81408_t *dev)
{
    ...
    spi_set_format(DAC81408_SPI_INSTANCE, 8, SPI_CPOL_0, SPI_CPHA_1, SPI_MSB_FIRST);
    ...
    uint16_t spiconfig = DAC81408_TEMPALM_EN(0) |
        DAC81408_DACBUSY_EN(0) |
        DAC81408_CRCALM_EN(0) |
        DAC81408_SOFTTOGGLE_EN(0) |
...
void dac81408_write_register(dac81408_t *dev, uint8_t reg, uint16_t wdata)
{
    ...
    spi_write_blocking(DAC81408_SPI_INSTANCE, &msb, 1);
    spi_write_blocking(DAC81408_SPI_INSTANCE, &lsb, 1);
...
uint16_t dac81408_read_register(dac81408_t *dev, uint8_t reg)
{
    ...
    spi_write_blocking(DAC81408_SPI_INSTANCE, &cmd, 1);
    spi_write_blocking(DAC81408_SPI_INSTANCE, &dummy, 1);
    spi_write_blocking(DAC81408_SPI_INSTANCE, &dummy, 1);
    _dac81408_tcsh_delay();
...
void dac81408_set_ch_enabled(dac81408_t *dev, int ch, bool state)
{
    ...
    if (state) {
        dac81408_write_register(dev, DAC81408_REG_DACPWDWN, dacpwdwn & ~mask);  // power up (bit = 0)
    } else {
        dac81408_write_register(dev, DAC81408_REG_DACPWDWN, dacpwdwn | mask);   // power down (bit = 1)
    }

    dev->dacpwdwn_cache = dac81408_read_register(dev, DAC81408_REG_DACPWDWN);
}

int dac81408_get_ch_enabled(dac81408_t *dev, int ch)
{
...
void dac81408_set_range(dac81408_t *dev, int ch, dac81408_range_t range)
{
    ...
    if (ch < 4) {
        dev->dacrange0_cache = (dev->dacrange0_cache & ~mask) | ((range << shift) & mask);
        dac81408_write_register(dev, DAC81408_REG_DACRANGE0, dev->dacrange0_cache);
    } else {
        dev->dacrange1_cache = (dev->dacrange1_cache & ~mask) | ((range << shift) & mask);
